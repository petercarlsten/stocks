      ...
      <ol className="flex flex-col gap-2">
        {ranked.map((s, i) => (
          <li key={s.symbol} className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-gray-900 text-sm font-medium truncate flex-1" title={s.name}>{s.name}</span>
            <WolfHover isPositive={s.gain >= 0}>
              <TrumpHover isNegative={s.gain < 0}>
                <span className={`text-sm font-medium shrink-0 ${s.gain >= 0 ? "text-green-600" : "text-red-500"}`}>
