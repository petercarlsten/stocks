          ...
          refreshStockData(s.symbol, s.name)
            .then(({ data, earningsDate, currency, symbol: corrected, marketState, exchangeTimezoneName, quoteType, navTimestamp, earningsResult, dividendRate, dividendYield, exDividendDate, dividendDate, dividends }) => {
              ...
              return {
                ...
                dividendRate: dividendRate ?? s.dividendRate,
                dividendYield: dividendYield ?? s.dividendYield,
                exDividendDate: exDividendDate ?? s.exDividendDate,
                dividends: dividends ?? s.dividends,
              };
            })
            .catch(() => s)
...
const cutoff1yr = new Date();
          ...
          {leaderboardEnabled && (
            <div className="hidden lg:block mt-3">
              <DashboardLeaderboard stocks={stocks.map(s => ({ symbol: s.symbol, name: s.name, data: s.data, purchases: s.purchases, currency: s.currency }))} usdRates={usdRates} columns={3} className="bg-white rounded-xl p-4 shadow-sm w-full" />
            </div>
          )}
          {topGainersEnabled && (
            <div className="hidden lg:block mt-3">
