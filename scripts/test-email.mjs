...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif">
  <div style="max-width:620px;margin:40px auto 60px;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,0.08)">
    <div style="background:linear-gradient(135deg,#6366f1 0%,#a855f7 50%,#10b981 100%);padding:28px 32px 24px">
      <table style="width:100%;border-collapse:collapse"><tr>
        ...
        <td style="vertical-align:middle;padding-left:14px">
          <p style="margin:0;color:white;font-size:20px;font-weight:800">My Portfolio Value for ${month}</p>
        </td>
      </tr></table>
    </div>
    <div style="padding:32px">
      ...
      <p style="margin:32px 0 0;color:#9ca3af;font-size:12px;text-align:center">Sent by My Portfolio · Report sent on the 1st of each month.</p>
    </div>
  </div>
</body>
</html>`;
...
const res = await fetch("https://api.resend.com/emails", {
  ...
  body: JSON.stringify({
    ...
    to: [TO],
    subject: `My Portfolio Value for ${month}`,
    html,
  }),
});
